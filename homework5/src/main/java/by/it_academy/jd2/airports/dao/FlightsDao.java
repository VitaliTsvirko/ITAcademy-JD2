...
public class FlightsDao implements IFlightsDao {
    ...
    public int getFlightsCountByAirportsCodeAndDates(String departureAirportCode, String arrivalAirportCode, LocalDate departureDate, LocalDate arrivalDate) throws IllegalAccessException {
        ...
        try (Connection connection = dataSource.getConnection()) {
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ps.setString(1, departureAirportCode);
                ps.setTimestamp(2, Timestamp.valueOf(departureDate.atTime(00, 00, 00)));
                ps.setTimestamp(3, Timestamp.valueOf(departureDate.atTime(23, 59, 59)));
                ps.setString(4, arrivalAirportCode);
                ps.setTimestamp(5, Timestamp.valueOf(arrivalDate.atTime(00, 00, 00)));
                ps.setTimestamp(6, Timestamp.valueOf(arrivalDate.atTime(23, 59, 00)));
    ...
    public List<Flights> getFlightsByAirportsCodeAndDates(Lang lang, String departureAirportCode, String arrivalAirportCode, LocalDate departureDate, LocalDate arrivalDate, int limit, int offset) throws IllegalAccessException {
        ...
        try (Connection connection = dataSource.getConnection()) {
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                ps.setTimestamp(3, Timestamp.valueOf(departureDate.atTime(00, 00, 00)));
                ps.setTimestamp(4, Timestamp.valueOf(departureDate.atTime(23, 59, 59)));
                ps.setString(5, arrivalAirportCode);
                ps.setTimestamp(6, Timestamp.valueOf(arrivalDate.atTime(00, 00, 00)));
                ps.setTimestamp(7, Timestamp.valueOf(arrivalDate.atTime(23, 59, 00)));
                ps.setInt(8, limit);
                ps.setInt(9, offset);
    ...
    public static void main(String[] args) throws PropertyVetoException, IllegalAccessException {
        ...
        Flights flights = flightsByAirportsCode.get(1);

        LocalDateTime localDateTime = flights.getScheduledArrival().toLocalDateTime();
